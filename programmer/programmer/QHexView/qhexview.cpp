...
void QHexView::undo() {
    if (m_hexdocument) m_hexdocument->undo();
}
void QHexView::redo() {
    if (m_hexdocument) m_hexdocument->redo();
}
...
void QHexView::copyAs(CopyMode mode) const {
    ...
    QByteArray bytes = m_hexcursor->hasSelection() ? m_hexcursor->selectedBytes() : m_hexdocument->read(m_hexcursor->offset(), 1);
...
unsigned int QHexView::calcAddressWidth() const {
    ...
    if (maxaddr <= std::numeric_limits<quint8>::max()) return 2;
    if (maxaddr <= std::numeric_limits<quint16>::max()) return 4;
...
QHexPosition QHexView::positionFromPoint(QPoint pt) const {
    ...
    switch (this->areaFromPoint(pt)) {
        ...
        case QHexArea::Ascii:
            pos.column = qMax<qint64>(qFloor((abspt.x() - this->asciiColumnX()) / this->cellWidth()) - 1, 0);
            break;
...
void QHexView::moveNext(bool select) {
    ...
    if (select)
        this->hexCursor()->select(qMin<qint64>(line, this->lines()), qMin<qint64>(column, this->getLastColumn(line) + offset));
    else
        this->hexCursor()->move(qMin<qint64>(line, this->lines()), qMin<qint64>(column, this->getLastColumn(line) + offset));
}
...
void QHexView::movePrevious(bool select) {
    ...
    if (select)
        this->hexCursor()->select(qMin<qint64>(line, this->lines()), qMin<qint64>(column, this->getLastColumn(line)));
    else
        this->hexCursor()->move(qMin<qint64>(line, this->lines()), qMin<qint64>(column, this->getLastColumn(line)));
}
...
bool QHexView::keyPressMove(QKeyEvent* e) {
    if (e->matches(QKeySequence::MoveToNextChar) || e->matches(QKeySequence::SelectNextChar))
        ...
        if (e->matches(QKeySequence::MoveToEndOfDocument))
            this->hexCursor()->move(this->lastLine(), this->getLastColumn(this->hexCursor()->line()));
        else
            this->hexCursor()->select(this->lastLine(), this->getLastColumn(this->lastLine()));
...
void QHexView::mousePressEvent(QMouseEvent* e) {
    ...
    switch (area) {
        case QHexArea::Address:
            this->hexCursor()->move(pos.line, 0);
            break;
        case QHexArea::Hex:
            m_currentarea = area;
...
void QHexView::wheelEvent(QWheelEvent* e) {
    ...
    if (ydelta > 0)
        this->verticalScrollBar()->setValue(this->verticalScrollBar()->value() - m_options.scrollsteps);
    else if (ydelta < 0)
        this->verticalScrollBar()->setValue(this->verticalScrollBar()->value() + m_options.scrollsteps);
}
